...
contains the truth value and bits 1 to 7 shall be zero\footnote{Other
bits are left unspecified, hence the consumer side of those values can
rely on it being 0 or 1 when truncated to 8 bit.}.  When an argument of
type \code{_Bool} is passed in a register or on the stack, bit 0
contains the truth value and bits 1 to 31 shall be zero.

When a value of type signed/unsigned \code{char} or \code{short}
is returned or passed in a register or on the stack, it shall be
sign/zero extended to signed/unsigned \code{int}.
